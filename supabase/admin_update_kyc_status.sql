...
-- FINAL RPC: Admin KYC Update (JSONB Payload)
-- This version uses a single "payload" object to avoid PostgREST cache issues.


-- Nuclear drop of all possible variations to ensure a clean slate
DROP FUNCTION IF EXISTS admin_update_kyc_status(uuid, text, text);
DROP FUNCTION IF EXISTS admin_update_kyc_status(text, text, text);
DROP FUNCTION IF EXISTS admin_update_kyc_status(jsonb);

CREATE OR REPLACE FUNCTION admin_update_kyc_status(payload jsonb)
RETURNS boolean
LANGUAGE plpgsql
